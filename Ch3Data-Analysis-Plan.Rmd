...
#Subset to include p4 in and p5 mother and primary caregiver data only
dyad.df_pcgm.p5 <- dyad.df[which(dyad.df$parent_type_p5==2 & dyad.df$pcg_p5 ==1),]
```
...
#need to get rid of "0" values as to not create -inf values
dyad.df$log.latency_3sec[dyad.df$latency_3sec == 0] <- .5
#creating new log transformed variable
dyad.df$log.latency_3sec <- log(dyad.df$log.latency_3sec)

with(dyad.df, hist(log.latency_3sec,breaks = 20,
                   xlab = "Latency to success in log seconds",
                   main = "hist dyad latency - /n log transformed"))

mean(dyad.df$log.latency_3sec, na.rm = TRUE)
sd(dyad.df$log.latency_3sec, na.rm = TRUE)
range(dyad.df$log.latency_3sec, na.rm = TRUE)
```
...
#### Longitudinal (ORCA) Analyses  
  
* Does cooperative understanding (measured via the Anticipation Paradigm) at 12-months predict frequency of initiating and/or responding behaviours in a cooperative task with a peer at 24-months, when controlling for the initiating/responding behaviour of the peer?  
**Hypothesis**: Theoretically, cooperative understanding should develop before cooperative action/ability as understanding is critical to cooperative action (Tomasello et al., 2005). It is possible that infants who demonstrate cooperative understanding earlier in life (by 12-months) engage in more meaningful cooperative exchanges earlier than those who do not show early cooperative understanding, and thus this experience fast tracks the developmental trajactory and enables them to display more coordinated behaviours (responding) at 24-months.    
* Does cooperative understanding (ORCA child only) at 12-months correlate/predict the latency to success score for 24-month-old the dyad, when controlling for the actions (initiating/responding behaviour) of the peer?  
**Hypothesis**: As described above, cooperative understanding should predict a shorter latency as this would be indicative of more skilled/developed cooperative ability.  
...
```{r Parent personality predict initiating behaviours - continuous DV - linear regression}
...
```
```{r Parent personality predict initiating behaviours - binary DV - logistic regression}
...
```
```{r Parent personality predict responding behaviours - continuous DV - linear regression}
...
```
```{r Parent personality predict responding behaviours - binary DV - logistic regresion}
...
* Does parent personality (Agreeableness) at 9 months predict child (ORCA) contribution when controlling for the peers social behaviours (affiliation/antagonism/contribution)?  
...
```{r parent personality predicting contribution - ordinal log regression}
...
```
```{r parent personality predicting contribution to warm up tidy - linear regression}
...
```{r correlations for child temperament and parent personality}
...
```
```{r RT cooperation table and matrix corr, set-options, echo=FALSE, cache=FALSE, fig.width=500, fig.length=500}
options(width = 500)
